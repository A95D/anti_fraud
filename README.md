...
2. [Архитектура](#архитектура-хранилища)
3. [База данных](#база-данных-банка)
4. [ETL-процесс](#реализация-etl-логики)
5. [Скоринговая модель](#risk_modelpy)
6. [Витрина данных и визуализация](#примеры-визуализации)
7. [Установка и запуск](#установка-и-настройка)
...
Логическая модель:
![er_bank](src/pictures/er_bank.jpg) 
...
Логическая модель:
![er_stg](src/pictures/er_stg.jpg) 

### 4. Слой `core`
Ядро - хранит очищенные и обогащенные данные.

Логическая модель:
![er_core](src/pictures/er_core.jpg) 
...
- `risk_model.py` - Модуль, содержащий класс `RiskScoringModel `для скоринговой модели оценки транзакций. агружает весовые коэффициенты из JSON-файла и рассчитывает для каждого клиента
и каждой транзакции:
